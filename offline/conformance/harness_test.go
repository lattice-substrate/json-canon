...
func TestOfflineReleaseGateDocumentation(t *testing.T) {
	...
	if !strings.Contains(releaseWorkflow, "git worktree add --detach") {
		t.Fatal("release workflow must build offline control binary from evidence source commit")
	}
}
...
func TestOfflineReplayEvidenceReleaseGate(t *testing.T) {
	...
	if err := replay.ValidateEvidenceBundle(evidence, matrix, profile, replay.EvidenceValidationOptions{
		...
		ExpectedGovernanceUmbrellaCommit: lookupEnvTrimmed("JCS_OFFLINE_GOVERNANCE_UMBRELLA_COMMIT"),
		ExpectedGovernanceLockSHA256:     lookupEnvTrimmed("JCS_OFFLINE_GOVERNANCE_LOCK_SHA256"),
		...
		RequireOfficialES6Proof:          replay.ProfileRequiresOfficialES6NumberCorpus(profile),
		ExpectedOfficialES6CorpusLines:   replay.OfficialES6CorpusFullLines,
		ExpectedOfficialES6CorpusSHA256:  replay.OfficialES6CorpusFullSHA256,
	}); err != nil {
