...
// NewAdapter constructs a container adapter with the provided command runner.
...
// Prepare runs pre-replay setup commands for a container node.
func (a *Adapter) Prepare(ctx context.Context, node replay.NodeSpec, bundlePath string, replayIndex int) error {
	...
	if err != nil {
		return fmt.Errorf("container prepare command: %w", err)
	}
	return nil
}

// RunReplay executes one replay command for a container node.
func (a *Adapter) RunReplay(ctx context.Context, node replay.NodeSpec, bundlePath string, evidencePath string, replayIndex int) error {
	if len(node.Runner.Replay) == 0 {
		return fmt.Errorf("container replay command is required")
	}
	_, err := a.runner.Run(ctx, node.Runner.Replay, commandEnv(node, bundlePath, evidencePath, replayIndex))
	if err != nil {
		return fmt.Errorf("container replay command: %w", err)
	}
	return nil
}

// Cleanup runs post-replay cleanup commands for a container node.
func (a *Adapter) Cleanup(ctx context.Context, node replay.NodeSpec, replayIndex int) error {
	...
	if err != nil {
		return fmt.Errorf("container cleanup command: %w", err)
	}
	return nil
}
