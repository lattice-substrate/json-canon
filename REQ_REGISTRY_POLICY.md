...
| OFFLINE-INFRA-001 | offline/README.md | Contracts | MUST | Infrastructure manifest schema (`offline/schema/infra-manifest.v1.json`) MUST exist and validate IaC repo identity (`infra_repo_url`, `infra_repo_commit`), provider lock digest (`provider_lock_sha256`), and per-host substrate records (`role`, `cloud_provider`, `region`, `instance_type`, `image_id`). |
