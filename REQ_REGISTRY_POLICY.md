...
| OFFLINE-SERVER-001 | offline/README.md | Contracts | MUST | Server-backed offline profiles (`offline/profiles/server-linux-x86_64.yaml`, `offline/profiles/server-linux-arm64.yaml`) MUST include `infra-substrate-binding` in `required_suites`, MUST enforce `hard_release_gate: true` and `min_cold_replays >= 5`, and validation MUST reject any evidence artifact that omits the required v1 infra-binding or native-host measurement fields for those profiles. |
