...
func newServerRunRecord(path string, opts serverEvidenceOptions, gitCommit, sourceRoot, lockSHA string) serverRunRecord {
	...
	record := serverRunRecord{
		...
		SourceGitTag:        opts.tag,
		RepoRoot:            opts.root,
		SourceRoot:          sourceRoot,
		OutputDir:           opts.outputDir,
		RunRecordPath:       path,
		AWSRegion:           opts.awsRegion,
		StateMode:           opts.state.Mode,
		StateBucket:         opts.state.Bucket,
		StateRegion:         opts.state.Region,
		StateLockTable:      opts.state.LockTable,
		StateKey:            opts.state.Key,
		InfraDir:            opts.infraDir,
		ProviderLockPath:    opts.lockFilePath,
		ProviderLockSHA256:  lockSHA,
		AMILockPath:         opts.amiLockPath,
		X86EvidencePath:     filepath.Join(opts.outputDir, "x86_64", "offline-evidence.json"),
		ArmEvidencePath:     filepath.Join(opts.outputDir, "arm64", "offline-evidence.json"),
